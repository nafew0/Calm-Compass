...
import { getAppHomePath, getPostAuthRedirect } from '@/utils/appRoutes'
...
const Register = () => {
  ...
  const { register, resendVerificationEmail, isAuthenticated, loading: authLoading, user } = useAuth()
  ...
  if (isAuthenticated) return <Navigate to={getAppHomePath(user)} replace />
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      if (result.emailVerificationRequired) {
        ...
        navigate(getPostAuthRedirect(result.user, redirectTo))
      }
    } else {
